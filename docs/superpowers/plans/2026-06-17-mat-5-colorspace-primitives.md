...
export function srgbToLinear(srgb: TSLNode): ShaderNodeObject<Node> {
  const lowSegment = pow(srgb, 1).div(12.92);
  const highSegment = pow(srgb.add(0.055).div(1.055), 2.4);
  // step(0.04045, srgb) == 1 where srgb >= 0.04045 -> pick the high segment.
  return mix(lowSegment, highSegment, step(0.04045, srgb));
}
...
export function linearToSrgb(linear: TSLNode): ShaderNodeObject<Node> {
  const lowSegment = pow(linear, 1).mul(12.92);
  const highSegment = pow(linear, 1 / 2.4).mul(1.055).sub(0.055);
  return mix(lowSegment, highSegment, step(0.0031308, linear));
}
```
