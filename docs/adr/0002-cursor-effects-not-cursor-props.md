...
SHA-137 gave LedWall a cursor reaction, and SHA-138 asked whether every shader should grow its own cursor entry point or whether cursor reactions should be standalone components, the way other libraries ship `<CursorRipples>`. We chose the standalone form: a reaction that makes sense over any scene ships as a cursor effect on the output stage, such as CursorRipple and the planned CursorSpotlight. A position prop belongs on a component only when the reaction needs that component's own internals, such as LedWall's dot positions or GodRays' ray sources, and cannot be done from outside. The prop is a position, never a cursor: it is named for what it is (`swellCenter`, `center`), and the cursor is one animation signal you can feed it.

SHA-138 will make every position prop accept the string `'cursor'`, resolved in `useAnimatablePoint` to the scene's shared cursor. Once that work lands, `<LedWall swellCenter="cursor" />` will need no hook or wrapper component. Until then, the current API is `focus={useCursor()}`. The planned shorthand stays explicit in the JSX, which is the property the ambient option below lacks.
...
- LedWall keeps its swell because it needs dot positions. SHA-138 will rename its props to `swellCenter`, `swellRadius`, and `swell` so the point is named for its reaction. Its brighten will become CursorSpotlight's job.
...
- A cursor effect that moves the image, such as CursorRipple's refraction, does it as a base-pass UV warp, which the output stage applies before any Effect's color transform. It bends only what the Sources drew, and an Effect mounted before it draws over the water unbent. This is the one place an Effect does not compose over every Effect before it. SHA-138 accepted the limit for its first slice and left intermediate render targets, which would lift it, to a later ticket.
