...
SHA-158, under SHA-138, made every position prop accept the string `'cursor'`, resolved in `useAnimatablePoint` to the scene's shared cursor, so `<LedWall swellCenter="cursor" />` needs no hook or wrapper component. The shorthand stays explicit in the JSX, which is the property the ambient option below lacks.

## Considered options

- **An ambient scene-level cursor** that any component with an unset position prop reads implicitly. Rejected: a static-looking component would turn interactive because of a sibling, the idle vote gets murkier, and it does nothing for Mode 2. The explicit case already takes one line, `swellCenter="cursor"`.
...
- LedWall keeps its swell because it needs dot positions. SHA-158 renamed its props to `swellCenter`, `swellRadius`, and `swell` so the point is named for its reaction. Its brighten became CursorSpotlight's job.
