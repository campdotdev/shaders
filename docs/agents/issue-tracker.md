...
Create a Linear issue on the Shaders team.

## When a skill says "fetch the relevant ticket"

Call `get_issue` with the `SHA-` identifier, then `list_comments` for the discussion.
...
- **Blocking**: Linear's native issue relations, which render as a visible blocked banner in the UI. Wire an edge with `save_issue` on the blocked ticket, passing `blockedBy: ["SHA-123"]`, or from the other side with `blocks`. Create the issues first and wire the edges in a second pass, because both ends need identifiers before they can reference each other. A ticket is unblocked when every issue blocking it sits in a closed state.
