...
`shape` also takes a custom mark as inline SVG markup: `{ svg: '<svg viewBox="0 0 24 24">…</svg>' }`. The browser decodes the markup once into a 128px tile, and the shader reads the tile's alpha at each grid point, so any fill in the markup is ignored and the mark takes `color`. The `viewBox` scales to fit `dotSize` on its longer side. Custom cells draw nothing on the first frame and appear once the decode lands, while built-in marks draw at once. The markup type is exported as `SvgMarkup`.
