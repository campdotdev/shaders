...
`CursorRippleShader`, the ripple's GPU half, is exported so a caller can render it over a wave field it drives itself. The wave field gains `tune()`, which changes its brush radius and its height and velocity damping while it runs, and `dampingForLifetime()`, which converts a ring's lifetime in seconds into a per-substep damping.
