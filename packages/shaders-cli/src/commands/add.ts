// `shaders-cli add`: the copy-paste flow the whole Tier 1 model is built on.
...
export async function runAdd(
  ...
  const componentsRoot = resolve(io.cwd, shadersConfig.componentsDir);
  ...
  const planned = await Promise.all(
    targets.map(async ({ file, targetPath }) => ({
      targetPath,
      contents: rewriteImports(
        await fetchComponentSource(registryUrl, file),
        shadersConfig.aliases,
      ),
    })),
  );
...
function resolveComponent(
  ...
  if (!entry) {
    throw new Error(
      `Component "${slug}" not found in registry at ${registryUrl}. Run \`shaders-cli list\` to see available components.`,
    );
  }
