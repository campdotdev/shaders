// Field-by-field validation of a parsed shaders.config.json, with error
// messages prefixed by the file path so a bad config points at itself.
import type { ShadersConfig } from './shadersConfig.js';

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null;
}

export function validateShadersConfig(parsed: unknown, path: string): ShadersConfig {
  if (!isRecord(parsed)) {
