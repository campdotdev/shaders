// The framework-free half of @camp-dev/shaders: TSL primitives, the renderer,
// the scheduler, and CPU color math. src/react and src/components import this
// barrel rather than the root index, which keeps the dependency direction one
// way (root -> react -> engine, root -> components -> react -> engine) and
// leaves this half extractable into its own package if a second framework
// binding ever becomes real.
...
export { createWaveField, dampingForLifetime } from './runtime/wave-field/wave-field.js';
export type {
  WaveField,
  WaveFieldStroke,
  WaveFieldTuning,
} from './runtime/wave-field/wave-field.js';
