...
11. **Consume vec-typed `uniform(...)` as an argument, not a chained receiver, in TSL math.** `uv().sub(cursorUniform)` works; chaining methods off a raw vec2/vec3 uniform node silently produces wrong GPU values despite typechecking. Build expressions from `uv()`/`vec2(...)` and pass vec uniforms as args. Scalar (float) uniforms are safe as chained receivers — waves chains them throughout and seven visual gates validated it.
