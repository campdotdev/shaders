...
- **Run user-facing prose through the technical-writing skill** (`~/.claude/skills/technical-writing`), which also applies `unslop` for the slop-pattern catalog. PR bodies, commit messages, changelog entries, release notes, and docs get a pass before the PR opens or the text ships. The skill sets `disable-model-invocation: true`, so Claude Code cannot reach for it on its own. Read its `SKILL.md` and work the review checklist at the end. Its first rule is to cut every word that does no work, so it pulls toward the PR word budget rather than against it. Diátaxis mode selection is the one layer a PR body skips.
- **Commit messages use Conventional Commits**, such as `feat(scope): …`, `fix(scope): …`, `chore: …`, `docs: …`, and `ci: …`. Scope is the package name without the `@lovo/` prefix.
...
- **The docs site consumes built `dist`, not source**, for `@lovo/matter` and `@lovo/matter-react`. `@matter/registry` is the exception, and reaches the site as raw `.tsx` via `transpilePackages`. After you edit engine or binding source, run `pnpm --filter @lovo/matter build` AND restart the docs dev server, or a correct fix looks like a no-op. Before you re-debug a "fix that didn't work," check `dist` mtime against `src`.
- **CI runs more than package-scoped checks.** Five traps:
  ...
  2. CI runs whole-repo Prettier through root `pnpm format:check`, not just lint. The import-sort plugin orders React and external imports before `@lovo/*` and `@matter/*`. Run Prettier on changed files before you commit.
  ...
  5. **`pnpm snap` and local Playwright runs corrupt a running docs dev server.** The Playwright `webServer` builds the production bundle into the same `apps/docs/.next` a dev server is serving from, and the Docker leg does it too, through the repo mount. The dev server then 500s every chunk with `Cannot find module './NNN.js'` from `webpack-runtime.js`. Compounding it, `reuseExistingServer: !process.env.CI` means the next local Playwright run reuses that broken server and every spec times out in `waitForShader`, which reads as a shader regression. It isn't one. Measured 2026-08-08, the same spec passed in the isolated Docker leg minutes earlier. Since MAT-94 this applies to BOTH apps, because the docs-tests webServer array builds `apps/editor/.next` too.
     ...
     1. Kill by port holder on the affected app's dev port. Docs is `lsof -ti tcp:3000` and editor is `lsof -ti tcp:3005`. Do both if both were running.
     2. Confirm zero holders on each port.
...
8. **The docs site needs `@matter/registry` plus `transpilePackages`** to import raw `.tsx` from a workspace dep.
9. **`three/webgpu` references `self` at module load, so it cannot SSR.** Anything that genuinely reaches the renderer needs `next/dynamic` with `{ ssr: false }`, and all eight component pages load their `scene.tsx` that way. Scalar code that merely ships in the same package does not, and each piece has an import path that never pulls in three: CPU color math at `@lovo/matter/color`, and `useDisplayGamut` at `@lovo/matter-react/gamut`. A `no-restricted-imports` rule scoped to `apps/docs/**` rejects both roots for those names, so the wrong import fails at lint rather than at render, and each subpath carries a `// @vitest-environment node` test that throws if three creeps back into its graph. Reach for a subpath before you reach for `ssr: false`.
