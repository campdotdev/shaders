...
- **Dev-only routes are named `page.dev.tsx` and need `INCLUDE_DEV_ROUTES=1`.** The docs site is `output: 'export'` with no route filtering, so anything the router can see lands in `out/` and gets indexed by `pagefind --site out` — unlinked debugging pages were showing up in site search on the deployed site. `next.config.ts` only adds `dev.tsx`/`dev.ts` to `pageExtensions` when that env var is `1`, so a plain `next build` cannot see them. Three things have to stay in sync if you touch this: `apps/docs`'s `dev` script sets the flag (so probes work locally), the Playwright `webServer` command sets it (four visual specs render against `/dev/*-probe`, and Playwright builds the PRODUCTION bundle rather than running the dev server — which is why `NODE_ENV` gating does not work here), and `turbo.json`'s `build` task declares `INCLUDE_DEV_ROUTES` under `env`, because turbo 2 runs strict env mode and would otherwise drop it before `next build` ever sees it. Verify a change by building both ways and checking whether `apps/docs/out/dev/` exists.
...
9. **`three/webgpu` references `self` at module load — cannot SSR.** Anything that genuinely reaches the renderer needs `next/dynamic` with `{ ssr: false }`; all eight component pages load their `scene.tsx` that way. Scalar code that merely ships in the same package does not, and each piece has a three-free door: CPU color math at `@lovo/matter/color`, `useDisplayGamut` at `@lovo/matter-react/gamut`. A `no-restricted-imports` rule scoped to `apps/docs/**` rejects both roots for those names, so the wrong import fails at lint rather than at render, and each subpath carries a `// @vitest-environment node` test that throws if three creeps back into its graph. Reach for a subpath before reaching for `ssr: false`.
