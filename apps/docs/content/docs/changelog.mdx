...
Shaders ships as three coordinated npm packages: `@camp-dev/shaders` (engine), `@camp-dev/shaders-react` (React binding), and `@camp-dev/shaders-cli` (copy-paste CLI). Each package has its own `CHANGELOG.md` in the repo, and this page summarizes the headline changes.
