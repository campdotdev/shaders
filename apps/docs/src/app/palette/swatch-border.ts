// Swatch borders for the palette page. A swatch whose color is close to the
...
// not the swatch, so it works for every color on the scale.
import { palette } from '@/lib/palette';

/**
 * A 1px border that reads against `surface`. Mid-neutrals rather than extremes:
 * strong enough to draw an edge, weak enough not to compete with the color it
 * frames.
 */
